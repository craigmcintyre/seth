 ...
 * error code and error message. The expected error message must be contained somewhere
 * within the actual error response. Case is sensitive.
 */
public class FailureErrorCodeAndMsgSubsetExpectedResult extends ExpectedResult
{
   ...
   */
  public FailureErrorCodeAndMsgSubsetExpectedResult(String description, OperationMetadata opMetadata,
                                                    AppContext appContext, Options options,
                                                    int expectedErrCode, String expectedErrMsg)
  {
    super(ExpectedResultType.FAILURE_CODE_AND_MSG_SUBSET, description, opMetadata, appContext, options);
  ...
  public void assertActualAsException(ExecutionContext xContext, SQLException e) throws FailureException
  {
    ...
    // Verify that the actual error message contains the expected error message.
