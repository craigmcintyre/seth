 ...
 * but with a particular warning message. The expected warning message must be the leading subset
 ...
 */
public class WarningMsgPrefixExpectedResult extends ExpectedResult
{
   ...
   */
  public WarningMsgPrefixExpectedResult(String description, OperationMetadata opMetadata,
                                        AppContext appContext, Options options, String expectedWarningMsg)
  {
    super(ExpectedResultType.WARNING_MSG_PREFIX, description, opMetadata, appContext, options);
  ...
  private void assertWarnings(SQLWarning warnings, ResultSet rs) throws FailureException
  {
    ...
    while (warn != null) {
      ...
      if (msg.startsWith(expectedWarningMsg)) {
        return;
      }
